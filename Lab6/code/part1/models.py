...
class GNN(nn.Module):
    def __init__(self, input_dim: int, hidden_dim_1: int, hidden_dim_2: int, hidden_dim_3: int, n_class: int):
        ...
        self.fc1 = nn.Linear(input_dim, hidden_dim_1)     # W0 (d,  h1)
        self.fc2 = nn.Linear(hidden_dim_1, hidden_dim_2)  # W1 (h1, h2)
        self.fc3 = nn.Linear(hidden_dim_2, hidden_dim_3)  # W2 (h2, h3)
        self.fc4 = nn.Linear(hidden_dim_3, n_class)       # W3 (h3, nclass)
        self.relu = nn.ReLU()
    ...
    def forward(self, x_in: torch.Tensor, adj: torch.Tensor, idx: torch.Tensor) -> torch.Tensor:
        ...
        """
        # GCN LAYERS
        # 2 x {Fully connected projection -> Message passing using the adjacency matrix -> Relu}
        z0 = self.relu(torch.mm(adj, self.fc1(x_in)))
        z1 = self.relu(torch.mm(adj, self.fc2(z0)))
        # READOUT
        # Squeeze the graph "spatial" dimension into a single [N, h2]
        ...
        # Fully connected projection -> Relu -> Fully connected projection
